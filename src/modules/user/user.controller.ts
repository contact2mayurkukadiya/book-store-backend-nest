...
// import { AdminCreatedResponse, AdminDeletedResponse, AdminUpdatedResponse, AdminUser, CreateAdminUser, CreateSuperAdminUser, RoleIdAndCreateBy, SearchAdminReq, UpdateAdminStatus, UpdateAdminUser, UpdateAdminViewAccountPermission } from 'src/models/user.model';
...
export class UserController {
    ...
    constructor(private userService: UserService, private utils: UtilsService, private queryService: QueryService) {
    ...
    //             const inserted: AdminUser = await this.userService.CreateUser(input);
        ...
        try {
            ...
            if (currentUser) {
                return this.utils.sendJSONResponse(res, HttpStatus.CONFLICT, {
                    success: false,
                    message: ERROR_CONST.EMAIL_ALREADY_EXIST,
                    data: null
                });
            }

            const input: CreateUser = body;
            const inserted = await this.userService.CreateUser(input);
            delete inserted.password;
            logger.log(level.info, `New User Created : ${this.utils.beautify(inserted)}`);
            return this.utils.sendJSONResponse(res, HttpStatus.OK, {
                success: true,
    ...
    //         const list: any = await this.userService.FindAdminByRoleIdAndCreatedId(filter);
    ...
    //             [APP_CONST.SUPER_ADMIN_ROLE]: [APP_CONST.ADMIN_ROLE, APP_CONST.SUB_ADMIN_ROLE]
    //         }
    //         const toBeDeleteAdmin = await this.userService.FindAdminById(id);
    ...
    //             const deleted = await this.userService.DeleteAdminQuery(id);
    ...
    //         const toBeUpdateAdmin = await this.userService.FindAdminById(updateId);
    //         if (toBeUpdateAdmin) {
    //             const json = toBeUpdateAdmin.permissions
    //             json['viewAccounts'] = body.viewAccountPermission;
    //             const updated = await this.userService.UpdateAdminQuery(updateId, { permissions: json });
    //             logger.log(level.info, `updated: ${this.utils.beautify(updated)}`);
    //             this.utils.sendJSONResponse(res, HttpStatus.OK, {
    //                 success: true,
    //                 statusCode: HttpStatus.OK,
